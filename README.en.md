...
- Output: **`firmware/imprint_left.uf2`** / **`firmware/imprint_right.uf2`** /
  `firmware/imprint_dongle.uf2` / `firmware/prospector.uf2` (git-ignored)
...
levels, received from the Imprint Dongle's BLE status advertisement. It only
listens: it never advertises, pairs or connects. It appears on the Mac only as
one CDC serial port (product `Prospector Dongle`, no HID keyboard; seen on
hardware 2026-09-26), and that port exists so that opening it at 1200 baud
enters the bootloader. The Imprint Dongle must run this repository's
...
whose README explains what the screen shows (`75%`; white `--` for a half
without a reading; grey `--` when nothing arrived for a minute).
