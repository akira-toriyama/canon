#!/usr/bin/env python3
"""host/chord/config.tmpl から host/chord/README.md のショートカット表を生成。
...
TMPL = ROOT / "host" / "chord" / "config.tmpl"
README = ROOT / "host" / "chord" / "README.md"

BEGIN = "<!-- AUTO-GENERATED (scripts/gen-chord-doc.py from host/chord/config.tmpl) — do not edit -->"
...
def build_block() -> str:
    ...
    if not rows:
        raise SystemExit(
            f"config.tmpl: # doc + [[bindings]] の組が見つかりません ({TMPL})"
        )
...
def update_readme(check_only: bool = False) -> int:
    ...
    if not pattern.search(text):
        raise SystemExit(
            f"{README} に AUTO-GENERATED マーカーが無い "
            f"({BEGIN} / {END})"
        )
...
if __name__ == "__main__":
    sys.exit(update_readme(check_only=("--check" in sys.argv[1:])))
